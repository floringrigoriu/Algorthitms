...
def count_henchmen(total_lambs, is_generous):
    if total_lambs< 3:
        return total_lambs
    result = [1 ,2] if is_generous else [1, 1]
    total_lambs-= sum(result)
    while total_lambs>= sum(result[-2:]):
        next_val = result[-1]<<1 if is_generous else sum(result[-2:]) 
        next_val = min(next_val,total_lambs)
        total_lambs-=next_val
        result.append(next_val)
    print('>' , total_lambs, result)
    return len(result)


def solution(total_lambs):
    # Your code here
    return count_henchmen(total_lambs, False)-count_henchmen(total_lambs, True)
